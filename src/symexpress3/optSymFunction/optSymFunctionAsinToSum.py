...
class OptSymFunctionAsinToSum( optFunctionBase.OptFunctionBase ):
  """
  Convert asin to sum
  """
  __slots__ = ()
  ...
  def __init__( self ) -> None :
    ...
    self._desc         = "Convert asin to sum"
    self._funcName     = "asin"                    # name of the function
  ...
  def optimize( self, elem:symexpress3.TypVarSym3Object, action:None|str ) -> None|symexpress3.TypVarSym3Object:
    ...
    elemSym = symexpress3.SymFormulaParser( f"sum( {varName}, 0 ,infinity, factorial( 2 * {varName} ) / ( exp( 2 * {varName}, 2) * factorial( {varName})^^2) * exp( ( 2 * {varName} ) + 1, x) / ( 2 * {varName} + 1) " )
...
def Test( display:bool = False) -> None :
  ...
  symTest:symexpress3.TypVarSym3Object = symexpress3.SymFormulaParser( "asin(pi/4)" )
  symTest.optimize()
  ...
  testClass = OptSymFunctionAsinToSum()
  symNew    = testClass.optimize( symTest, "asinToSum" )
  ...
  if str( symNew ).strip() != "sum( n1,0,infinity, factorial( 2 * n1 ) * ( exp( 2 * n1,2 ) *  factorial( n1 )^^2)^^-1 *  exp( 2 * n1 + 1,pi * 1 * 4^^-1 ) * (2 * n1 + 1)^^-1 )":
