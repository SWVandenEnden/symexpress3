...
class OptSymFunctionSinXplusYtoSinCos( optFunctionBase.OptFunctionBase ):
  """
  ...
  def optimize( self, elem, action ):
    ...
    elemStr = '( sin(' + strX + ') * cos(' + strY + ') + cos(' + strX + ')* sin( ' + strY + ') )'
...
def Test( display = False):
  ...
  symNew    = testClass.optimize( symTest, "sinXplusYtoSinCos" )
  ...
  if str( symNew ).strip() != "( sin( pi * 1 * 4^^-1 ) *  cos( 5 * 1 * 2^^-1 ) +  cos( pi * 1 * 4^^-1 ) *  sin( 5 * 1 * 2^^-1 ))":
